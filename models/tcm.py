...
class TCM(CompressionModel):
    def __init__(self, config=[2, 2, 2, 2, 2, 2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0, N=128,  M=320, num_slices=5, max_support_slices=5, **kwargs):
        ...
        self.g_a = nn.Sequential(
            *[ResidualBlockWithStride(1, 2*N, 2)]
             + self.m_down1
             + self.m_down2
             + self.m_down3
        )
    ...
    def forward(self, x):
        y = self.g_a(x)
        y_shape = y.shape[2:]
        z = self.h_a(y)             #16,192,4,4
        _, z_likelihoods = self.entropy_bottleneck(z)

        z_offset = self.entropy_bottleneck._get_medians()   #全0？192,1,1
        z_tmp = z - z_offset
        z_hat = ste_round(z_tmp) + z_offset                 #取整
    ...
    def compress(self, x):              #1,1,896,2048
        y = self.g_a(x)                 #1,320,56,128
        y_shape = y.shape[2:]

        z = self.h_a(y)                 #1,192,14,32
        z_strings = self.entropy_bottleneck.compress(z)
        z_hat = self.entropy_bottleneck.decompress(z_strings, z.size()[-2:]) #1,192,14,32

        latent_scales = self.h_scale_s(z_hat)               #1,320,56,128
        latent_means = self.h_mean_s(z_hat)                 #1,320,56,128
        y_slices = y.chunk(self.num_slices, 1)              # 5* 1,64,56,128
        y_hat_slices = []
        y_scales = []
        y_means = []

        cdf = self.gaussian_conditional.quantized_cdf.tolist() #64*3133
        cdf_lengths = self.gaussian_conditional.cdf_length.reshape(-1).int().tolist() #64
